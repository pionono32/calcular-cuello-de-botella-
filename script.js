...
function getResultClass(diagnosis) {
  if (diagnosis === "CPU bottleneck") return "state-cpu";
  if (diagnosis === "GPU bottleneck") return "state-gpu";
...
function renderMain() {
  ...
  // diagnosis
  if (diagnosisText) diagnosisText.textContent = result.diagnosis;
  const diagBadge = document.getElementById("diagnosis-badge");
  if (diagBadge) {
    const labels = { "CPU bottleneck": "⚠️ CPU limita", Balanced: "✅ Balanceado", "GPU bottleneck": "🎮 GPU limita" };
    const sp = diagBadge.querySelector("span");
    if (sp) sp.textContent = labels[result.diagnosis] || result.diagnosis;
  }

  // confidence
  if (confidencePill) confidencePill.textContent = confidenceLabels[result.confidence];
  const confDot = document.getElementById("conf-dot");
  if (confDot) confDot.className = "conf-dot" + (result.confidence !== "high" ? " " + result.confidence : "");

  // numbers
  if (avgFpsValue)    avgFpsValue.textContent    = `${result.average}`;
  if (rangeValue)     rangeValue.textContent     = `${result.range.min} — ${result.range.max}`;
  if (lowValue)       lowValue.textContent       = `${result.low1}`;
  if (gpuUsageValue)  gpuUsageValue.textContent  = `${result.gpuUsage}%`;
  if (explanationText) explanationText.textContent = result.explanation;
  if (referenceText)  referenceText.textContent  = result.reference;

  resultCard.className = `result-card ${getResultClass(result.diagnosis)}`;
  if (avgBar) avgBar.style.width = `${clamp((result.average / 280) * 100, 4, 100)}%`;
  if (lowBar) lowBar.style.width = `${clamp((result.low1   / 230) * 100, 4, 100)}%`;
  if (gpuBar) gpuBar.style.width = `${result.gpuUsage}%`;

  // bottleneck bar (nuevo HTML)
  const btFill = document.getElementById("bottleneck-fill");
  if (btFill) btFill.style.width = `${clamp(result.bottleneckPercent / 70 * 100, 0, 100)}%`;
  const btPct  = document.getElementById("bottleneck-percent");
  if (btPct)  btPct.textContent  = `${result.bottleneckPercent}%`;

  // gauge viejo (por si existe)
  if (bottleneckGauge) {
    const gaugeNorm = Math.min(result.bottleneckPercent / 70 * 100, 100);
    bottleneckGauge.style.setProperty("--gauge-value", `${gaugeNorm}%`);
    const gMap = { "CPU bottleneck": "var(--red)", Balanced: "var(--yellow)", "GPU bottleneck": "var(--green)" };
    bottleneckGauge.style.setProperty("--gauge-color", gMap[result.diagnosis]);
  }
}
...
// ─── CHATBOT FLOTANTE ────────────────────────────────────────────────────────
//
// IMPORTANTE: La API de Anthropic requiere una API key enviada desde el servidor.
// Desde un HTML local no funciona por CORS + seguridad de la key.
// Para producción: usar un proxy backend (Cloudflare Worker, Vercel Edge, etc.)
// que agregue el header "x-api-key" y reenvíe la request a api.anthropic.com.
// ─────────────────────────────────────────────────────────────────────────────

const CHAT_SYSTEM = `Sos un asistente general muy capaz, integrado en una herramienta de análisis de rendimiento gaming. Respondés en español argentino, de forma clara, directa y sin paja.

Podés responder CUALQUIER tipo de pregunta: hardware, gaming, tecnología, programación, cultura, ciencia, historia, matemáticas, idiomas, o lo que sea. No tenés restricciones temáticas.

Cuando el contexto lo requiera, tenés acceso a la configuración actual del usuario en la herramienta (CPU, GPU, juego, resolución, RAM) y podés usarlo para dar consejos más precisos.

Estilo: conciso, sin relleno. Si la pregunta es técnica, respondé técnico. Si es casual, respondé casual. Máximo 5 oraciones salvo que el tema lo requiera.`;
...
function openChat() {
  ...
  panel.classList.add("open");           // new CSS uses .open
...
async function sendChatMessage() {
  ...
  // primer mensaje: inyectar contexto de la herramienta
  let content = txt;
  if (!chatHistory.length) {
    content = `[Contexto herramienta: CPU=${cpuSelect.value}, GPU=${gpuSelect.value}, Juego=${gameSelect.value}, Resolución=${resolutionSelect.value}, RAM=${ramCapacitySelect.value} ${ramSpeedSelect.value} ${ramChannelSelect.value}]\n\n${txt}`;
  }

  addMsg(msgs, txt, "user-msg");

  const typing = document.createElement("div");
  typing.className = "msg bot-msg typing-msg";
  ...
  try {
    ...
    typing.remove();
    addMsg(msgs, reply, "bot-msg");

  } catch (err) {
    typing.remove();
    const isLocal = location.protocol === "file:";
    const msg = isLocal
      ? "El chatbot requiere que el sitio esté hosteado en un servidor web con proxy backend para la API key. No funciona con file://."
      : `Error de conexión: ${err.message}`;
    addMsg(msgs, msg, "bot-msg error-msg");
  }
...
function addMsg(container, text, classes) {
...
function initChatbot() {
  ...
  document.querySelectorAll(".chat-chip").forEach(chip => {
    chip.addEventListener("click", () => {
      document.getElementById("chatbot-input").value = chip.textContent.trim();
